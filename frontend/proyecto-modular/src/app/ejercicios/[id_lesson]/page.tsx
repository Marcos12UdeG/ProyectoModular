...
export default function ExercisesPage() {
  ...
  useEffect(() => {
    ...
    const fetchExercises = async () => {
      setLoading(true);
      try {
        const res = await fetch(`http://localhost:8000/lessons/${id_lesson}/exercises_with_answers`);
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center p-6">
      <h1 className="text-4xl font-extrabold text-[#3E2723] mt-6 mb-8 text-center tracking-wide">
        📚 Ejercicios de la Lección {id_lesson}
      </h1>

      {loading ? (
        <p className="text-center text-gray-500">Cargando ejercicios...</p>
      ) : exercises.length === 0 ? (
        <p className="text-center text-gray-500">No hay ejercicios para esta lección.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 w-full max-w-7xl">
          {exercises.map((exercise) => (
            <div
              key={exercise.id_excercise}
              className="bg-white/95 rounded-3xl shadow-2xl hover:shadow-[0_12px_40px_rgba(0,0,0,0.2)] transition-all p-6 flex flex-col"
            >
              <div className="flex justify-between items-center mb-3">
                <h2 className="text-xl font-bold text-[#4E342E]">{exercise.excercise_name}</h2>
                ...
                {exercise.answers.length > 0 ? (
                  exercise.answers.map((answer) => (
                    <button
                      ...
                      onClick={() => handleSelectAnswer(exercise.id_excercise, answer.id_answer)}
                      className={`px-4 py-2 rounded-lg border text-left transition-all ${
                        selectedAnswers[exercise.id_excercise] === answer.id_answer
                  ...
                  <p className="text-sm text-gray-500">No hay respuestas disponibles.</p>
                )}
              </div>

              {selectedAnswers[exercise.id_excercise] &&
                exercise.answers.some(a => a.id_answer === selectedAnswers[exercise.id_excercise] && a.is_correct !== undefined) && (
                  <p className="mt-2 text-sm font-semibold">
                    {exercise.answers.find(a => a.id_answer === selectedAnswers[exercise.id_excercise])?.is_correct
                      ? "✅ Correcta"
                      : "❌ Incorrecta"}
                  </p>
                )}
